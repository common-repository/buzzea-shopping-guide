...
return array(
'quarterhourly' => array('interval' => 900, 'display' => 'Once Every Fifteen Minutes'),
);
}
...
function gill_check_activation() {
	wp_schedule_event(time(), 'quarterhourly', 'gill_check_event');
}
function gill_check_update() {
	...
	if(gill_plugin_get("Version") >= $version ) return false; //== changed to >= GF
...
/*if (is_admin() && $_SERVER['REMOTE_ADDR'] == '78.XXX.10.XXX'){
	global $wpdb;		
	$options_table_name = "`{$wpdb->prefix}options`";
	$sql = "SELECT * FROM $options_table_name WHERE option_name = '_site_transient_update_plugins' ";
	//echo $sql;
