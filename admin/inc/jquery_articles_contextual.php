<?php 
/*
Description: This file contains the admin jquery functions for contextuelle articles in the Buzzea WP Comparateur de Prix
Author: Buzzea.com
...
*/
	$articlesnonceadd = wp_create_nonce( 'articlescontextuellenonceadd' );
	...
	$articles_showarticlescategory_contextuelle_nonce	= wp_create_nonce( 'articlesnonceshowarticlescategorycontextuelle' );
	$articlesnonceadd 									= wp_create_nonce( 'articlescontextuellenonceadd' );
	$contextuelle_editornonce 							= wp_create_nonce( 'contextuelle_editornonce' );
	$widgetcontextuellenonceupdate						= wp_create_nonce( 'widgetcontextuellenonceupdate' );
	$articlesnoncemultipledelete						= wp_create_nonce( 'noncemultipledelete' );
	$articlesnoncealldelete								= wp_create_nonce( 'noncealldelete' );
	
	$articlesnoncedelete = wp_create_nonce( 'articlescontextuellenoncedelete' );
...
?><script type="text/javascript">
	...
	if (isNaN(postid)){
		window.cdp_target_path = '<?php echo $cdp_target_path; ?>';
	} else {
		window.cdp_target_path = '/wp-admin/post.php?post=' + postid;
	}
	
	function cdp_refresh_iframe(){
		// hackishly force iframe to reload
		var iframe = document.getElementById('cdp_iframe_widget_autox150');
		iframe.src = iframe.src;	
	}
	
	/*
	* check a box on admin looking for arrival of the permalink (new articles)
	*/
	function cdp_perma_check(){
			
		permalink_check = jQuery('#edit-slug-box').html();
		permalink_search = permalink_check.search(':'); // -1, not found
		//make global
		window.cdp_permalink_available = permalink_search;
		console.log('check');
		//try to show...
		//	cdp_show_contextuelle_editor();
	}
	
	/*
	 * display contextuelle editor
	*/
	function cdp_show_contextuelle_editor(){
		if (window.cdp_permalink_available!=-1 && window.cdp_permalink_available!=false){
		
			var postid = parseInt(jQuery('#post_ID').val());
			
			// kill off interval checking
			clearInterval(window.cdp_myInterval);
			
			// get the content via ajax
			jQuery.ajax({
				type: "post",url: "<?php echo CDP_AJAX_URL; ?>",data: { action: 'cdp_getcontextuelle_editor', target_path: window.cdp_target_path, postid: postid, _ajax_nonce: '<?php echo $contextuelle_editornonce; ?>' },
				beforeSend: function() { 
					
					// loading...
					jQuery('#cdp_meta_box_contextuelle_loader_holder').show('fast');
					jQuery('#cdp_meta_box_contextuelle_loader').show('fast'); // make visible*/
					
					}, 
				success: function(html){  //if data is retrieved, store it in html
					
					window.contextuelle_editor_displayed = true;
					
					if (html != ''){
						// successful 
						jQuery('#cdp_meta_box_contextuelle_loader_holder').hide('fast'); //hide loader
						jQuery('#cdp_meta_box_contextuelle').html(html); //results -  the meta box
						jQuery('#cdp_meta_box_contextuelle_notice').hide(); //remove note
					} else {
	...
	jQuery(document).ready(function(jQuery) {
		...
		// to allow draft saving, in order to show the contextual editor (need permalink)
		jQuery('.metabox_submit').click(function(e) {
			e.preventDefault();
			
			//permalink_check = jQuery('#edit-slug-box').html();
			permalink_check = jQuery('#title').val();
			
			//permalink_search = permalink_check.search(':'); // -1, not found
			if (permalink_check != ''){ permalink_search = 1; } else { permalink_search = -1; }
			
			if (permalink_search == -1){
				alert('<?php echo __("S\'il vous plaît ajouter un titre à votre article / page et enregistrez", CDP_PLUGIN_NAME ); ?>')	
			} else {
				jQuery('#publish').click(); //submit in order to save draft
			}
		}); 

		// init
		window.cdp_permalink_available = false;
		
		/*********************************************
			ajax call to fill contextuelle editor
		 ********************************************/
		window.contextuelle_editor_displayed = false;
		
		if (window.contextuelle_editor_displayed == false){
			
			/*var iFrequency = 5000; // expressed in miliseconds, check every 5 secs for the permalink
			var cdp_myInterval = 0;
			
			if(cdp_myInterval > 0) clearInterval(cdp_myInterval);  // stop
    		window.cdp_myInterval = setInterval( "perma_check()", iFrequency );  // run*/
			
			cdp_show_contextuelle_editor(); //display editor
		...
		*/
		...
		jQuery("a[class^='cdp_cat_arts_add_']").live('click', function(e) { 
				...
				// cdp_show_cat_article_js(catid);
				cdp_add_article_contextuelle_js(jQuery, artid, '<?php echo $articlesnonceadd; ?>', '<?php echo $article_add_success_txt; ?>', '<?php echo $article_add_fail_txt; ?>');
		...
		jQuery("a[class^='cdp_cat_arts_del_']").live('click', function(e) { 
				...
				if (cdp_confirmAction("<?php echo __("Etes-vous sur de vouloir supprimer cet article?", CDP_PLUGIN_NAME ); ?>")){
	 				cdp_delete_article_contextuelle_js(artid);
			    } else { return false; }
		});
		 ...
		 * Actions groupée
		*/
		//1.9 new jQuery(document).on('click', "[id=cdp_group_actions_apply]", function(e) {
		 ...
		 * duplicate function
		*/
		//1.9 new jQuery(document).on('click', "[id*=select_all_cdc_]", function(e) {
		...
		*/
		function multipleDelArticleEnSelection(articleids){
			...
			if (cdp_confirmAction("<?php echo __("Êtes-vous sûr de vouloir supprimer ces articles ?", CDP_PLUGIN_NAME ); ?>")){
				...
				<?php $permalink = cdp_get_draft_permalink(); // needed for reference ?>
				
				//ajax call to delete multiple articles
				jQuery.ajax({
					type: "post",url: "<?php echo CDP_AJAX_URL; ?>",data: { action: 'cdp_delete_multiple_articles_contextuelle', articleids:articleids, contexte_url: '<?php echo $permalink; ?>',  _ajax_nonce: '<?php echo $articlesnoncemultipledelete; ?>' },
					beforeSend: function() { 
					...
					success: function(html){  // if data is retrieved, store it in html
						...
						// successful 
						cdp_show_articles_contextuelle_js();
		...
		*/
		function DelTousArticleEnSelection(){
			if (cdp_confirmAction("<?php echo __("Êtes-vous sûr de vouloir supprimer tous ces articles ?", CDP_PLUGIN_NAME ); ?>")){
				...
				//ajax call to delete multiple articles
				jQuery.ajax({
					type: "post",url: "<?php echo CDP_AJAX_URL; ?>",data: { action: 'cdp_delete_all_articles_contextuelle', contexte_url: '<?php echo $permalink; ?>',  _ajax_nonce: '<?php echo $articlesnoncealldelete; ?>' },
					beforeSend: function() { 
						/*jQuery('#setting-error-settings_error').remove(); jQuery('#setting-error-settings_updated').remove(); /* clear notice area */ 
						jQuery("#cdp_articles_list").fadeOut('fast'); 
						jQuery("#cdp_articles_list_loading").fadeIn('fast');
	...
	function cdp_show_cat_article_js(categorieid,more) {
		...
		jQuery.ajax({
			type: "post",url: "<?php echo CDP_AJAX_URL; ?>",data: { action: 'cdp_showarticles_for_category_contextuelle', categorieid: categorieid, limitfrom: limitfrom, limitfor: limitfor, target_path: window.cdp_target_path, _ajax_nonce: '<?php echo $articles_showarticlescategory_contextuelle_nonce; ?>' },
			beforeSend: function() { 
				jQuery('#setting-error-settings_error').remove(); jQuery('#setting-error-settings_updated').remove(); /* clear notice area */ 
	 ...
	 ***********************************
		takes: jQuery (equivalent of $)
				...
				articlesnonceadd
	*/
	function cdp_add_article_contextuelle_js(jQuery, articleid, articlescontextuellenonceadd, successtxt, failtxt) {
		jQuery.ajax({
			type: "post",url: "<?php echo CDP_AJAX_URL; ?>",data: { action: 'cdp_addarticlecontextuelle', articleid: articleid, target_path: window.cdp_target_path, _ajax_nonce: articlescontextuellenonceadd },
			...
			success: function(html){  //if data is retrieved, store it in html
				...
				if (html == 'existe'){
					...
					setTimeout('cdp_show_articles_contextuelle_js()', '800');
					
					//refresh the category articles div
	...
	jQuery(function(){
		...
		jQuery("#cdp_addarticlesbox").autocomplete({
			...
			select: function(e, ui) {
				...
				//add article
				cdp_add_article_contextuelle_js(jQuery, chosenarticleid, '<?php echo $articlesnonceadd; ?>','<?php echo $article_add_success_txt; ?>','<?php echo $article_add_fail_txt; ?>' ); 
...
//loading articles with ajax inspired by http://ocaoimh.ie/wp-content/uploads/2008/11/helloworld3.txt / http://ocaoimh.ie/2008/11/01/make-your-wordpress-plugin-talk-ajax/
  ...
  $articlesnonce = wp_create_nonce( 'articlescontextuellenonce' );
...
<script type="text/javascript">
	 ...
	 ***********************************
		takes: nothing
	*/
	function cdp_show_articles_contextuelle_js(  ) {
		jQuery.ajax({
			type: "post",url: "<?php echo CDP_AJAX_URL; ?>",data: { action: 'cdp_getarticlescontextuelle', limitstart: '<?php echo $limitstart; ?>', limitfor: '<?php echo $limitfor; ?>', paging: '<?php echo $paging; ?>',  target_path: window.cdp_target_path,  _ajax_nonce: '<?php echo $articlesnonce; ?>' },
	 ...
	 ***********************************
		takes: articleid
	*/
	function cdp_delete_article_contextuelle_js( articleid ) {
		jQuery.ajax({
			type: "post",url: "<?php echo CDP_AJAX_URL; ?>",data: { action: 'cdp_deletearticlecontextuelle',articleid: articleid, target_path: window.cdp_target_path, _ajax_nonce: '<?php echo $articlesnoncedelete; ?>' },
			...
			success: function(html){ //so, if data is retrieved, store it in html
				...
				setTimeout('cdp_show_articles_contextuelle_js()', '1200');
				/*jQuery("#cdp_articles_list_loading").fadeOut('fast');
				jQuery("#cdp_articles_list").fadeIn("fast"); //animation*/
				//jQuery("#cdp_addarticlesbox").focus();
	...
	*/	
	jQuery(document).ready(function(jQuery) {
		...
		/* delete an article - start */
			...
			jQuery("a[class^='deletearticle_']").live('click', function(e) { 
				e.preventDefault(); // this stops it from firing the link and is the same as having return false; at the end of the function
				  ...
				  cdp_delete_article_contextuelle_js(divIDnum);
				  //alert (divIDnum);
		...
		*/
		...
		// watching for activation / deactivation
		jQuery("input[id^='banner_contextuelle_active_']").live('click', function(e) { 
			...
			widgets[widgetsize] = '';
			// banner_contextuelle_active_autox150
			var active = jQuery('#banner_contextuelle_active_'+widgetsize).filter(':checked').val();
			if (typeof active != 'undefined') {
				widgets[widgetsize] = { 'active': 1 };
				
			} else {
				widgets[widgetsize] = { 'active': 0 }; //inactive 
			}
			
			widgetdata = JSON.stringify(widgets);
			
			widgeturl = jQuery('#cdp_widget_contextuelle_url_'+widgetsize).val();
			
			cdp_update_widget_contextuelle_js(jQuery, widgetsize, widgetdata, widgeturl, '<?php echo $widgetcontextuellenonceupdate; ?>', '<?php echo 'success_txt'; ?>', '<?php echo 'fail_txt'; ?>');
		});
		
		/*********************************** 
			ajax call to update a contextuelle widget
		 ***********************************
			takes: jQuery (equivalent of $)
					widgetid, 
					widgetnonceadd
		*/
		function cdp_update_widget_contextuelle_js(jQuery, widgetsize, widgetdata, widgeturl,  widgetcontextuellenonceupdate, successtxt, failtxt) {
			
			jQuery.ajax({
				type: "post",url: "<?php echo CDP_AJAX_URL; ?>",data: { action: 'cdp_updatewidget_contextuelle', widgetsize: widgetsize, widgeturl: widgeturl, _ajax_nonce: widgetcontextuellenonceupdate, widgetdata: widgetdata  },
				beforeSend: function() { 
