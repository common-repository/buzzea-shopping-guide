<?php 
/*
Description: This file contains the admin jquery functions for articles in banners in the Buzzea WP Comparateur de Prix
Author: Buzzea.com
...
?><script type="text/javascript">
	...
	*/
	...
	jQuery(document).ready(function(jQuery) {
		...
		*/
		jQuery("a[id^='cdp_cat_arts_']").live('click', function(e) { 
				...
				// what domaine id?
				table = $this.closest('.cdp_enselection_table');
				domaineid = table.attr('rel');
				more = false;

				cdp_show_cat_article_js(catid, more, domaineid);
		...
		*/
		jQuery("a[class^='cdp_cat_arts_add_']").live('click', function(e) { 
				...
				// what domaine id?
				domaineid = $this.attr('rel');
				
				// cdp_show_cat_article_js(catid);
				cdp_add_article_widget_js(jQuery, artid, '<?php echo $articlesnonceadd; ?>', '<?php echo $article_add_success_txt; ?>', '<?php echo $article_add_fail_txt; ?>', domaineid);
		...
		*/
		jQuery("a[class^='cdp_cat_arts_del_']").live('click', function(e) { 
				...
				if (cdp_confirmAction("<?php echo __("Etes-vous sur de vouloir supprimer cet article ?", CDP_PLUGIN_NAME ); ?>")){
	 				cdp_delete_article_js(artid, domaineid);
			    } else { return false; }
		});
		...
		*/
		...
		jQuery("[id^=cdp_group_actions_apply_]").live('click', function(e) { 
			//function groupactionCdp(){ 
			
			// what domaine id?
			domaineid = jQuery(this).attr('rel');
			...
			if (selectval == 'delete'){
				...
				multipleDelArticleEnSelection(valeurs, domaineid);
				} else {
					alert('<?php echo __("Aucune article en selection", CDP_PLUGIN_NAME ); ?>');	
					 ...
					 DelTousArticleEnSelection(domaineid); //del tous
			} else {
				//rien
			}
		...
		*/
		function multipleDelArticleEnSelection(articleids, domaineid = FALSE){
			...
			if (cdp_confirmAction("<?php echo __("Êtes-vous sûr de vouloir supprimer ces articles ?", CDP_PLUGIN_NAME ); ?>")){
				...
				jQuery.ajax({
					type: "post",url: "<?php echo CDP_AJAX_URL; ?>",data: { action: 'cdp_delete_multiple_articles', domaineid:domaineid, articleids:articleids,  _ajax_nonce: '<?php echo $articlesnoncemultipledelete; ?>' },
					beforeSend: function() { 
					...
					success: function(html){  // if data is retrieved, store it in html
						...
						// successful 
						cdp_show_articles_js(domaineid);
					}
				}); //close jQuery.ajax(
		...
		*/
		function DelTousArticleEnSelection( domaineid = FALSE){
			if (cdp_confirmAction("<?php echo __("Êtes-vous sûr de vouloir supprimer tous ces articles ?", CDP_PLUGIN_NAME ); ?>")){
				
				//ajax call to delete multiple articles
				jQuery.ajax({
					type: "post",url: "<?php echo CDP_AJAX_URL; ?>",data: { action: 'cdp_delete_all_articles', domaineid:domaineid,  _ajax_nonce: '<?php echo $articlesnoncealldelete; ?>' },
					beforeSend: function() { 
						/*jQuery('#setting-error-settings_error').remove(); jQuery('#setting-error-settings_updated').remove(); /* clear notice area */ 
						jQuery("#cdp_articles_list").fadeOut('fast'); 
						jQuery("#cdp_articles_list_loading").fadeIn('fast');
		...
		*/
		jQuery('.cdp_articles_per_cat').scroll(function() {
			...
			divWidth 		= jQuery('#cdp_articles_per_cat_holder_'+domaineid).width();
			divContentWidth = jQuery(this).get(0).scrollWidth; // jQuery('#cdp_articles_per_cat').width();
			scrollLeftPosn 	= jQuery(this).scrollLeft();
			...
			if( scrollLeftPosn + divWidth >  divContentWidth - 200 ) {
			   ...
			   if (window.more_articles_called == false && window.no_more_articles == false	){
				    ...
				    cdp_show_cat_article_js(categorieid,more=true,domaineid);
	...
	/*
	 * fill div with articles available for category
	 */
	function cdp_show_cat_article_js(categorieid,more,domaineid) {
		...
		jQuery('#cdp_articles_per_cat_'+domaineid).show('fast');
		
		
		//ajax call for more articles
		jQuery.ajax({
			type: "post",url: "<?php echo CDP_AJAX_URL; ?>",data: { action: 'cdp_showarticles_for_category', domaineid:domaineid, categorieid: categorieid, limitfrom: limitfrom, limitfor: limitfor, _ajax_nonce: '<?php echo $articles_showarticlescategory_nonce; ?>' },
			beforeSend: function() { 
				jQuery('#setting-error-settings_error').remove(); jQuery('#setting-error-settings_updated').remove(); /* clear notice area */ 
				/*jQuery('#cdp_articles_per_cat_loader_holder').show('show');
				jQuery('#cdp_articles_per_cat_loader').show('slow'); // make visible*/
				
				jQuery('#cdp_articles_per_cat_loader_holder_'+domaineid).show('show');
				jQuery('#cdp_articles_per_cat_loader_'+domaineid).show('slow'); // make visible
			...
			success: function(html){  //if data is retrieved, store it in html
				...
				jQuery('#cdp_articles_per_cat_loader_holder_'+domaineid).delay(500).hide('show');
				jQuery('#cdp_articles_per_cat_loader_'+domaineid).delay(500).hide('slow'); // make invisible
				
				jQuery('#cdp_articles_per_cat_'+domaineid).show('slow'); // make visible
				...
				if (html != ''){
					...
					if (more != true ) {
						...
						jQuery('#cdp_articles_per_cat_'+domaineid).html(''); // blank it (remove prog bar)
					}
					if (html == 'pas_articles'){
						...
						if (window.articles_first_call == true){
							...
							jQuery('#cdp_articles_per_cat_'+domaineid).html(html);	
					...
					prev_html 	= jQuery('#cdp_articles_per_cat_'+domaineid).html();	 // accumulate new html
					new_html 	= prev_html + html;
					jQuery('#cdp_articles_per_cat_'+domaineid).html(new_html);			// update html to div
	...
	/*********************************** 
		ajax call to add an article to a widget/banner
	 ***********************************
		takes: jQuery (equivalent of $)
				articleid, 
				articlesnonceadd
	*/
	function cdp_add_article_widget_js(jQuery, articleid, articlesnonceadd, successtxt, failtxt, domaineid) {
		
		jQuery.ajax({
			type: "post",url: "<?php echo CDP_AJAX_URL; ?>",data: { action: 'cdp_addarticle', articleid: articleid, domaineid:domaineid, _ajax_nonce: articlesnonceadd },
			...
			success: function(html){  //if data is retrieved, store it in html
				...
				if (html == 'existe'){
					...
					setTimeout('cdp_show_articles_js('+domaineid+')', '800');
					
					//new - refresh the category articles div
					if (!isNaN(window.current_catid)){
						//global current id
						catid = window.current_catid;
						setTimeout('cdp_show_cat_article_js('+catid+', '+'null'+', '+domaineid+')', '800');
	...
	jQuery(function(){
		...
		jQuery("#cdp_addarticlesbox").autocomplete({
			...
			source: function(req, add){ //, add
				...
				// what domaine id?
				table = jQuery("#cdp_addarticlesbox").closest('.cdp_enselection_table');
				domaineid = table.attr('rel');
				
				jQuery.getJSON('<?php echo CDP_AJAX_URL;?>?action=get_search_results_admin&domaineid='+domaineid, req, function(data) {
					add(data);
				});
			...
			select: function(e, ui) {
				...
				cdp_add_article_widget_js(jQuery, chosenarticleid, '<?php echo $articlesnonceadd; ?>','<?php echo $article_add_success_txt; ?>','<?php echo $article_add_fail_txt; ?>', domaineid ); 
...
<script type="text/javascript">
	...
	/*********************************************** 
		ajax call to show articles already selected
	 ***********************************************
		takes: nothing
	*/
	function cdp_show_articles_js( domaineid ) {
		
		// need domaineid to get articles for this specific widget
		if (isNaN(domaineid)){
			// do nothing
		} else {
			//alert('getting articles for '+domaineid);
			
			jQuery.ajax({
				type: "post",url: "<?php echo CDP_AJAX_URL; ?>",data: { action: 'cdp_getarticles', domaineid: domaineid, limitstart: '<?php echo $limitstart; ?>', limitfor: '<?php echo $limitfor; ?>', paging: '<?php echo $paging; ?>',  target_path: '<?php echo $cdp_target_path; ?>',  _ajax_nonce: '<?php echo $articlesnonce; ?>' },
	 ...
	 ***********************************
		takes: articleid
	*/
	function cdp_delete_article_js( articleid, domaineid ) {
		jQuery.ajax({
			type: "post",url: "<?php echo CDP_AJAX_URL; ?>",data: { action: 'cdp_deletearticle',articleid: articleid,domaineid: domaineid, _ajax_nonce: '<?php echo $articlesnoncedelete; ?>' },
			...
			success: function(html){ //so, if data is retrieved, store it in html
				...
				setTimeout('cdp_show_articles_js('+domaineid+')', '1200');
				
				//new - refresh the category articles div
				if (!isNaN(window.current_catid)){
					//global current id
					catid = window.current_catid;
					more = 'null'; //not actually null
					setTimeout('cdp_show_cat_article_js('+catid+', '+more+', '+domaineid+')', '800');
				}
			}
		}); //close jQuery.ajax(
	}
	...
	jQuery(document).ready(function(jQuery) {
		...
		/* show articles - start */
			...
			jQuery('[id^="cdp_widgets_table_opener_"]:not(.cdp_delete_banner_btn)').live('click', function(e) { 
				...
				if (jQuery('#cdp_widgets_table_'+rel).is(":visible")){
					//get the articles for this table you just clicked on to open
					cdp_show_articles_js(rel);
				}
		...
		/* delete an article - start */
			...
			jQuery("a[class^='deletearticle_']").live('click', function(e) { 
				e.preventDefault(); // this stops it from firing the link and is the same as having return false; at the end of the function
				  ...
				  domaineid = $this.attr('rel');
				  //alert(domaineid);
				  cdp_delete_article_js(divIDnum, domaineid);
				  //alert (divIDnum);
